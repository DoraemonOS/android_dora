function __print_dora_functions_help() {
cat <<EOF
...
- pixelgerrit:     A Git wrapper that fetches/pushes patch from/to doraemon Gerrit Review.
- pixelrebase:     Rebase a Gerrit change and push it again.
- doraremote:      Add git remote for matching dora repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for doraemon Github.
...
- repopick:        Utility to fetch changes from doraemon Gerrit.
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop org.doraemon.device | grep -q "$DORA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $DORA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function doraremote()
{
    ...
    git remote add dora https://android.googlesource.com/$PFX$PROJECT
    echo "Remote 'dora' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/DoraemonOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.doraemon.device | grep -q "$DORA_BUILD");
    then
...
function pixelgerrit() {
    ...
    local user=`git config --get review.gerrit.doraemonos.org.username`
...
function pixelrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "doraemon Gerrit Rebase Usage: "
    ...
    git fetch "http://gerrit.doraemonos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop org.doraemon.device | grep -q "$DORA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/dora/build/tools/repopick.py $@
}
