...
def get_from_manifest(device_name):
    if os.path.exists(dora_local_manifest):
        man = load_manifest(dora_local_manifest)
...
def add_to_manifest(repos, fallback_branch=None):
    lm = load_manifest(dora_local_manifest)
    ...
    for repo in repos:
        ...
        if 'branch' in repo:
            ...
            repo_branch=dora_default_revision
        if 'remote' in repo:
...
def fetch_dependencies(repo_path, fallback_branch=None):
    ...
    dep_p = '/'.join((repo_path, dora_dependencies))
    ...
    for dependency in dependencies:
        if not is_in_manifest(dependency['target_path']):
            if not dependency.get('branch'):
                dependency['branch'] = dora_default_revision
...
def detect_revision(repo):
    ...
    print("Calculated revision: %s" % dora_default_revision)

    if has_branch(result, dora_default_revision):
        return dora_default_revision

    print("Branch %s not found" % dora_default_revision)
    sys.exit()
